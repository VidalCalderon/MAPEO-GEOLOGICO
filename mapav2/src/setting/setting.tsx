...
export default class Setting extends React.PureComponent<MapSettingProps, State> {
  ...
  onDisableSelection = (useDataSources: UseDataSource[]): boolean => {
    // Permitir hasta 20 mapas
    if (useDataSources.length > 19) {
      return true
    } else {
      return false
    }
  ...
  onClickDisabledDsItem = () => {
    if (this.props.useDataSources && this.props.useDataSources.length >= 20) {
      setTimeout(() => {
    ...
    return (
      ...
      <div className='widget-setting-map'>
        ...
        <SettingSection className='section-title' title={this.props.intl.formatMessage({ id: 'sourceLabel', defaultMessage: defaultMessages.sourceLabel })}>
          ...
          {
            // Thumbnails dinámicos para cualquier número de mapas (1 a 10)
            portalUrl && this.props.dsJsons && this.props.useDataSources && this.props.useDataSources.length >= 1 &&
            <SettingRow>
              <div className='w-100 d-flex flex-wrap justify-content-start' style={{ gap: '6px' }}>
                {this.props.useDataSources.map((uds, index) => {
                  const dsId = uds?.dataSourceId
                  const dsJson = dsId ? this.props.dsJsons[dsId] : null
                  const portalUrlForDs = dsJson?.portalUrl || null
                  const mapItemId = dsJson?.itemId || null
                  const label = dsJson?.label || dsJson?.sourceLabel || ''
                  const isSelected = this.props.config.initialMapDataSourceID === dsId
                  const thumbClass = this.props.useDataSources.length === 1
                    ? 'webmap-thumbnail selected-item'
                    : classNames('webmap-thumbnail-multi', { 'selected-item': isSelected })
                  return (
                    <div
                      key={dsId || index}
                      onClick={() => { this.setInitialMap(dsId) }}
                      title={label}
                      className={thumbClass}
                      tabIndex={0}
                      role='button'
                      onKeyDown={e => { this.handleMapThumbKeyDown(e, index) }}
                    >
                      <MapThumb
                        usedInSetting={true}
                        portUrl={portalUrlForDs}
                        mapItemId={mapItemId}
                        label={label}
