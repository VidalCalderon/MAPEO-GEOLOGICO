...
// 4. Add handleAddToTable right before handleOpacityChange
...
// 5. Replace menu in renderUserLayerTree
const menuSearchStr = `{hasMenuOpen && (
...
// 6. Rewrite the ENTIRE return function to apply layout + table changes
let lines = content.split('\n');
let returnStartIndex = -1;
for (let i = 0; i < lines.length; i++) {
    if (lines[i] === "  return (" && !lines[i-1].includes("renderUserLayerTree")) {
        // found the main return
        // double check if it's the main return by seeing if it's towards the end of file
        if (i > 700) {
            returnStartIndex = i;
            break;
        }
